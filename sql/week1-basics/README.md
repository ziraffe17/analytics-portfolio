...
```sql
HAVING AVG(score) >= 80  -- 平均点80点以上
```

#### Pattern 3: WHEREとHAVINGの組み合わせ
```sql
WHERE attendance_date >= '2025-01-01'  -- 集約前フィルタ
GROUP BY user_id
HAVING COUNT(*) >= 5                    -- 集約後フィルタ
   AND COUNTIF(status = 'present') * 100.0 / COUNT(*) >= 60
```

#### Pattern 4: 重複データの検出
```sql
HAVING COUNT(*) >= 2  -- 重複メールアドレスの検出
```
...
| 対象 | 行単位 | グループ単位 |
| 使える関数 | 通常の関数 | 集約関数 |
| 用途 | データの絞り込み | グループの絞り込み |

**実務での活用:**
- パターン1-2: KPIレポート（目標達成者の抽出）
- パターン3: 複合条件フィルタ（優良利用者の特定）
- パターン4: データ品質チェック（重複検出）
- パターン5: 顧客セグメント（VIP顧客の抽出）
- パターン6: ベンチマーク分析（平均以上のパフォーマンス）

**ポートフォリオへの応用:**
- プロジェクト1（教育データ）: 優秀な学生の抽出、科目別トップ校の分析
- プロジェクト2（あゆみSaaS）: 目標達成者レポート、要支援者の早期発見
...
### 5. WHERE vs HAVINGの使い分け（NEW）
- **WHERE**: 行を減らしてから集約（パフォーマンス向上）
- **HAVING**: 集約してからグループを絞る（ビジネスロジック）
- **両方使う**: WHEREで事前フィルタ → GROUP BY → HAVING で最終絞り込み
- **実務では**: 常に処理順序を意識して、効率的なクエリを書く

### 6. BigQuery特有の注意点
（既存の内容に以下を追加）
- HAVINGでも集約関数を使える（AVG, SUM, COUNT等）
- サブクエリとの組み合わせで強力な分析が可能
...
### 今日（10/19）残りのタスク
- ✅ Task24: 集約関数の応用（完了）
- ✅ Task25: HAVING句の活用（完了）
- ⬜ Task26: LeetCode Easy 3問（577/584/586）
- ⬜ Task40: Kaggle教育データ詳細分析

### 明日（10/20月）
- Task27-29: ウィンドウ関数総仕上げ + LeetCode
- Week 1完了レビュー
- Week 2準備

---

**最終更新**: 2025年10月21日（Task24-25完了）  
**累計学習時間**: 約9時間  
**次のタスク**: Task26（LeetCode Easy 3問）
```
...
`人生設計シート シート5.csv` で以下を完了に変更：
```
TRUE	Task24	Week1	2025-10-19	日	集約関数の応用学習	30	学習	High		学習ノート
TRUE	Task25	Week1	2025-10-19	日	HAVING句の活用	30	実践	High	Task24	SQLファイル
```
...
### Week 1 進捗（10/19時点）
```
Week 1（10/11-10/20）: SQL基礎固め
✅ Task01-14: 環境構築・基礎学習（完了）
✅ Task15-19: ウィンドウ関数（完了）
✅ Task20: LeetCode Easy 3問（完了）
✅ Task21-22: CTE基礎（完了）
✅ Task23: LeetCode Easy 3問（完了）
✅ Task24: 集約関数の応用（完了）← NEW
✅ Task25: HAVING句の活用（完了）← NEW
⬜ Task26: LeetCode Easy 3問（今日中）
⬜ Task27-29: ウィンドウ関数総仕上げ（10/20）
